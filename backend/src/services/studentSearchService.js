...
const buildStudentSearchQuery = (params) => {
  ...
  if (minRatingScore !== null) {
    query.ratingScore = { $gte: minRatingScore };
  }

  if (params.country) {
    query.country = String(params.country).trim();
  }
  ...
  if (params.hasInternship === 'true') {
    query.$expr = { $gt: [{ $size: '$internships' }, 0] };
  }

  if (params.hasInternship === 'false') {
    query.$expr = { $eq: [{ $size: '$internships' }, 0] };
  }

  if (minProjectCount !== null) {
    query.$expr = {
      $and: [query.$expr || { $literal: true }, { $gte: [{ $size: '$projects' }, minProjectCount] }],
    };
  }

  if (minAwardsCount !== null) {
    query.$expr = {
      $and: [query.$expr || { $literal: true }, { $gte: [{ $size: '$awards' }, minAwardsCount] }],
    };
  }
